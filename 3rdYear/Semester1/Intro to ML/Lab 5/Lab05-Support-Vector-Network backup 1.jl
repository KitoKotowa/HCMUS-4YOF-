...
function pla(pos_data::Matrix{Float64}, neg_data::Matrix{Float64}, 
	...
	for i in 1:n_epochs
		for j in randperm(size(data)[2])
			if check(θ,data[:,j],label[j])
				θ = update_theta(θ,data[:,j],label[j])
			end
		end 
		loss_ = loss(sign(compute(θ,data)),label')
...
function eval_pla(θ, pos_data, neg_data)
	...
	tpfptnfn_cal(y_test,y_pred)

	# Catcutate precision, recatt, and Fl-score
...
And we have : $\theta^{(k+1)} = \theta^{(k)} + y_t\mathbf{x}_t$ with $t = 1...n$

At k+1 : $Q(k+1) =(\theta^{*})^\top\theta^{(k+1)} = (\theta^{*})^\top(\theta^{(k)} + y_t\mathbf{x}_t) = (\theta^{*})^\top\theta^{(k)} + y_t(\theta^{*})^\top x_t \geq Q(k) + \gamma$

Conclusion : $Q(k)$ will increase by a maximum of $\gamma$ for each update, or increase at least linearly of $\gamma$
...
And : $\theta^{(k+1)} = \theta^{(k)} + y_t\mathbf{x}_t$ với $t = 1...n$

At k+1 : $P(k+1) =(\theta^{(k+1)})^\top\theta^{(k+1)} = (\theta^{(k)} + y_t\mathbf{x}_t)^\top(\theta^{(k)} + y_t\mathbf{x}_t) = ((\theta^{k})^\top + y_t\mathbf{x}_t^\top)(\theta^{(k)} + y_t\mathbf{x}_t)$  \
...
Conclusion : $P(k)$ increases at most linearly in the number of updates $\mu$, or increase no more than  linear rate of $\mu$
...
We have $l_1:{\theta}^\top\mathbf{x}+b=1$ and $l_2:{\theta}^\top\mathbf{x}+b=-1$ are 2 parallel hyperplanes in the same hyperspace (having ${\theta}^\top$ as the normal vector)  \
Given any $\mathbf{x_0}$ that belongs to hyperplane $l_1:{\theta}^\top\mathbf{x}+b=1$ we will have $\mathbf{x_0}=\frac{-b+1}{{\theta}^\top}$
...
- n_epochs::Int64=10000: Maximum training epochs. Default is 10000
"""
function hardmargin_svm11(pos_data, neg_data, η=0.04, n_epochs=10000)
	# START YOUR CODE
	...
	println(b)
		...
		#neg_label = -ones(size(neg_data)[2])
		#data = [hcat(pos_data, neg_data);ones(size(pos_data)[2]+size(neg_data)[2])']
		#θ = rand(size(data)[1])
	## Loss function
...
# ╔═╡ 5e71e887-f097-44ec-8321-d3c147c26c5a
...
- n_epochs::Int64=10000: Maximum training epochs. Default is 10000
"""
function softmargin_svm11(pos_data, neg_data, n_epochs=10000, C=0.12, η=0.01)
	# START YOUR CODE
...
# ╔═╡ c5e37344-14e7-4bf6-b2db-30fe1ee54ad7
...
function dualsvm_smo_hard(pos_data, neg_data, n_epochs=100, λₜₒₗ=0.0001, errₜₒₗ=0.0001)
	...
	D = ([
	  shuffle(tuple.(eachcol(pos_data), 1))
	  shuffle(tuple.(eachcol(neg_data), -1))
	])	
...
# ╟─3fdaee93-9c4f-441a-9b4a-4c037f101955
# ╟─665885b7-9dd7-4ef9-8b5b-948295c20851
# ╠═b4541480-b3b4-4986-9ad9-4f0946df96f3
# ╠═c5e37344-14e7-4bf6-b2db-30fe1ee54ad7
# ╠═eb0f6469-a0dd-4a6b-a3c2-6916c58072a9
# ╠═d531768a-0aef-43ae-867b-f1670211e06f
# ╠═f79e78e5-27d6-43be-bb32-4066dba0d373
