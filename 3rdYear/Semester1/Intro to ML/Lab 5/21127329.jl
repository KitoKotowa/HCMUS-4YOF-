...
function hardmargin_svm(pos_data, neg_data, η=0.04, n_epochs=10000)
    ...
    for i in 1:n_epochs
		...
		if all_good
			b = b + (min_distance(w,b,neg_data,neg_label) -min_dictance(w,b,pos_data,pos_label))/2
			#Gradient descent
...
function dualsvm_smo_hard(pos_data, neg_data, n_epochs=100, C = 1000, λₜₒₗ=0.0001, errₜₒₗ=0.0001)
	# START YOUR CODE
	...
	function process(j)
		...
		error_j = errors[j]
	 	r_j = error_j*y_j
		if ((r_j < -errₜₒₗ) || (r_j > errₜₒₗ && alpha_j_old > 0))
...
# ╔═╡ e0703634-ee1f-45a6-bccc-45a23c67ae78
