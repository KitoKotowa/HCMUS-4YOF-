...
--use master drop database QLDT1
----------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------
...
where not exists(select* 
				from GIAOVIEN gv2, DETAI dt, THAMGIADT tg
				where gv2.MAGV = tg.MAGV and dt.TENDT = N'Quản lý giáo dục')
	and gv.MABM = bm.MABM and k.MAKHOA = bm.MAKHOA and k.MAKHOA = N'CNTT'
...
where exists(select* 
				from GIAOVIEN gv2, DETAI dt, THAMGIADT tg
				where gv2.MAGV = tg.MAGV and dt.TENDT = N'Thành phố thông minh')
	 and bm.TRUONGBM = gv.MAGV and not exists (SELECT *
											  FROM BOMON bm, KHOA k
										      WHERE bm.TRUONGBM = GV.MAGV and k.MAKHOA = bm.MAKHOA and k.MAKHOA = N'CNTT') 
...
SELECT GV.*
FROM GIAOVIEN GV
WHERE NGSINH = (SELECT MIN(GV.NGSINH) FROM GIAOVIEN GV, BOMON BM
				WHERE BM.TRUONGBM = GV.MAGV)

--Câu 5
SELECT  GV.HOTEN, COUNT(*) as TONGDETAI
FROM GIAOVIEN GV,DETAI DT, THAMGIADT TG, BOMON BM, KHOA K, CHUDE CD
WHERE GV.MABM = BM.MABM and K.MAKHOA = K.MAKHOA and K.MAKHOA = N'CNTT'
AND gv.MAGV = tg.MAGV and CD.TENCD = N'Quản lý giáo dục'
GROUP BY GV.HOTEN

--Câu 13
SELECT distinct	gv1.MAGV MAGV1, GV2.MAGV  MAGV2
FROM GIAOVIEN gv1, GIAOVIEN gv2, THAMGIADT t1, THAMGIADT t2
WHERE gv1.MAGV = t1.MAGV 
and gv2.MAGV = t2.MAGV 
and t1.MAGV = gv1.MAGV 
and t2.MAGV = gv2.MAGV 
and gv1.MAGV <>  gv2.MAGV
group by gv1.MAGV, gv2.MAGV
having count(distinct t1.MADT) = count(distinct t2.MADT)


--Câu 6
SELECT gv.MAGV, gv.HOTEN
FROM GIAOVIEN gv, DETAI d, THAMGIADT t, KHOA k, BOMON bm, CHUDE c
WHERE  gv.MABM = bm.MABM and bm.MAKHOA = k.MAKHOA and k.MAKHOA = N'CNTT'
		and t.MADT = d.MADT
		and c.MACD = d.MACD
		and c.TENCD = N'Quản lý giáo dục'	
		and gv.MAGV = t.MAGV
GROUP BY gv.MAGV, gv.HOTEN 
HAVING count(distinct t.MADT)  >=  ALL(select count(distinct t.MADT)
										from GIAOVIEN gv, DETAI d, THAMGIADT t, KHOA k, BOMON bm, CHUDE c
										where gv.MABM = bm.MABM and bm.MAKHOA = k.MAKHOA and k.MAKHOA = N'CNTT'
										and t.MADT = d.MADT
										and c.MACD = d.MACD
...
FROM GIAOVIEN gv  
WHERE gv.NGSINH = (select min(gv1.NGSINH)
				   from GIAOVIEN gv1, BOMON bm
				   where bm.MABM = gv1.MABM)
...
SELECT bm.TENBM
FROM BOMON bm
WHERE not exists(select bm1.*
				from BOMON bm1, THAMGIADT t, DETAI d
				where t.MAGV = bm1.TRUONGBM and t.MADT = d.MADT
				and year(d.NGAYBD) <> 2011)


--Câu 9
SELECT distinct gv1.MAGV
FROM GIAOVIEN gv1,BOMON bm, THAMGIADT t, DETAI d, KHOA k
WHERE gv1.MABM = bm.MABM
and bm.MAKHOA = k.MAKHOA
and gv1.MAGV <> k.TRUONGKHOA
and t.MAGV = gv1.MAGV
and t.MADT = d.MADT
and k.TRUONGKHOA = d.GVCNDT

--Câu 10
SELECT AVG(year(getdate()) - year(gv.NGSINH)) TuoiTB
FROM GIAOVIEN gv, BOMON bm, GIAOVIEN gv2, THAMGIADT t, CHUDE c, DETAI d
WHERE
		gv.MABM <> gv2.MABM
...
FROM GIAOVIEN gv, KHOA k, BOMON bm
WHERE EXISTS(select*
		from THAMGIADT t, CHUDE c, DETAI d
		where c.MACD = d.MACD
		and c.TENCD = N'Quản lý giáo dục'	
		and gv.MAGV = t.MAGV)
...
FROM GIAOVIEN gv
WHERE not exists(select *
			from CHUDE c, DETAI d, THAMGIADT t
			where	c.TENCD =  N'Quản lí giáo dục' 
					and d.MACD = c.MACD 
					and t.MADT = d.MADT 
					and gv.MAGV = t.MAGV
					)
		and not exists (select *
						from GIAOVIEN GV1
						where GV1.HOTEN = N'Nguyễn Văn An' and gv.GVQLCM = gv1.MAGV)

--Câu 14
SELECT d.MADT, d.TENDT
FROM DETAI d
WHERE exists(select gv.*
		from BOMON bm, GIAOVIEN gv, GIAOVIEN gv2, THAMGIADT t
		where gv.MABM = gv2.MABM
		and gv2.HOTEN = N'Trần Trà Hương'
		and t.MADT = d.MADT and t.MAGV = gv.MAGV)
...
WHERE bm.MABM = gv.MABM
		and gv.MAGV in	(select t.MAGV
							from THAMGIADT t)
		and tbm.MAGV = bm.TRUONGBM