...
SELECT GV.HOTEN, GV.LUONG * 1.1 as LUONGMOI
FROM GIAOVIEN GV
...
WHERE LUONG > 2000 AND GV.HOTEN LIKE N'Nguyễn %' OR BM.TRUONGBM = GV.MAGV AND YEAR(BM.NGAYNHANCHUC) > 1995
...
--Cau 10 
SELECT DT.MADT, CD.TENCD, GV.HOTEN, GV.DIACHI, GV.NGSINH 
FROM DETAI DT, CHUDE CD, GIAOVIEN GV
WHERE DT.GVCNDT = GV.MAGV AND DT.CAPQL = N'Thành phố' 

--Cau 11
SELECT GV.HOTEN AS GV, QL.HOTEN AS NGUOIQUANLY
...
WHERE CV.MADT = DT.MADT AND CV.NGAYBD BETWEEN '2008-03-01' AND '2008-03-31' AND DT.TENDT = N'HTTT quản lý các trường ĐH'
...
SELECT CV.TENCV
FROM DETAI DT, CONGVIEC CV
--WHERE CV.MADT = DT.MADT AND DT.NGAYBD BETWEEN '2008-01-01' AND '2008-01-08' 
WHERE CV.MADT = DT.MADT AND CV.NGAYBD BETWEEN '2008-01-01' AND '2008-08-01'

--Cau 18
SELECT DISTINCT GV.HOTEN 
FROM GIAOVIEN GV, GIAOVIEN TTH, BOMON BM
WHERE BM.MABM = GV.MABM AND BM.MABM = TTH.MABM AND TTH.HOTEN = N'Trần Trà Hương'
...
WHERE BM.TRUONGBM = GV.MAGV AND K.TRUONGKHOA = GV.MAGV
...
FROM GIAOVIEN GV, DETAI DT, KHOA K
WHERE K.TRUONGKHOA = GV.MAGV AND DT.GVCNDT = GV.MAGV

--Cau 23
SELECT DISTINCT GV.MAGV 
FROM GIAOVIEN GV, DETAI DT, BOMON BM, THAMGIADT TGDT
WHERE BM.TENBM = N'Hệ thống thông tin' AND BM.MABM = GV.MABM OR DT.MADT = TGDT.MADT AND DT.MADT = '001' AND TGDT.MAGV = GV.MAGV  

--Cau 24
SELECT DISTINCT GV.*
FROM GIAOVIEN GV, GIAOVIEN X, BOMON BM
WHERE BM.MABM = GV.MABM AND BM.MABM = X.MABM AND X.MAGV = '002'
...
--Cau 27
SELECT COUNT(*) TONGGV , SUM(LUONG) TONGLUONG
FROM GIAOVIEN 

--Cau 28
SELECT COUNT(DISTINCT MAGV) SOGV, AVG(LUONG) LUONGTB
FROM GIAOVIEN GV
GROUP BY GV.MABM
...
--Cau 30
SELECT GV.HOTEN, COUNT(DISTINCT TG.MADT) SODETAI
FROM GIAOVIEN GV, THAMGIADT TG 
WHERE GV.MAGV = TG.MAGV
GROUP BY GV.MAGV, GV.HOTEN

--Cau 31
SELECT GV.HOTEN, COUNT(DISTINCT DT.MADT) SODTCHUNHIEM
FROM GIAOVIEN GV, DETAI DT
WHERE GV.MAGV = DT.GVCNDT
GROUP BY GV.MAGV, GV.HOTEN

--Cau 32
SELECT GV.HOTEN, COUNT(NT.TEN) SONGUOITHAN
